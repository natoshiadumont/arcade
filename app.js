...
function randomGridLocation() {
   ...
   //determine that the  newFruitPostion does not go on a worm segment or is value of null
   while (newFruitPosition === null || wormAte(newFruitPosition)) {
      newFruitPosition = getNewLocation();
   }
...
function moveFruit() {
   if (wormAte(fruit)) {
      ...
      fruit = randomGridLocation();
      // fruit.id = randomFruitImg();
      scoreText.innerHTML = `Score: ${currentScore}`;
   }
   // console.log(fruit);
}
...
function renderFruit(gameGrid) {
   ...
   console.log("DEBUG:  calling renderFruit");
   if (fruit && fruit.x && fruit.y && fruit.id) {
      console.log("DEBUG: fruit position ", fruit.x, fruit.y)
      const fruitElement = document.createElement('div');
